# 6th Grade Computer Science Project
# Pine Crest School
# March 28, 2019
# CircuitPlayground Express Heartrate Sensor with BPM Detection
...
def get_bpm(samples):
    ...
    if len(beat_times) > 0:
        ...
        bpm_method2 = None
    bpm_method1 = beats * (60 / HISTORY_SECONDS)
...
while True:
    ...
    for i in range(NUM_SAMPLES):
        if monotonic() > last_time + 1/SAMPLE_HZ:
            ...
            history.pop()
            if samples[i] - mean > .60 * max(history[:2*int(SAMPLE_HZ)]):
                pixels[5] = (255, 0, 255)
            elif samples[i] - mean < .50 * max(history[:2*int(SAMPLE_HZ)]):
                pixels[5] = (0, 0, 0)
            if i == 0:
                bpm1, bpm2 = get_bpm(history)
            pixels.show()
            print((samples[i] - mean, max(history[:2*int(SAMPLE_HZ)]) * 0.6, monotonic() - last_time, bpm1, bpm2))  # 'center' the reading
